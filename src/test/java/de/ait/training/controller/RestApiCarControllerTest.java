...
//@WebMvcTest- поднимает слой контроллера
@WebMvcTest(controllers = RestApiCarController.class)
class RestApiCarControllerTest {

    @Autowired
    //MockMvc- отправляет на этот слой запросы
    private MockMvc mockMvc;

    //делаем подделку на репозиторий
    @MockitoBean
    ...
    //carRepository получаешь запрос на метод findCarByPriceBetween(с параметрами:10000.0, 30000.0)
        given(carRepository.findCarByPriceBetween(10000.0, 30000.0))
              //то возвращаешь 3 авто,которые подх. под наши параметры.В db незаходи, верни 3 авто подходящии
        ...
        mockMvc.perform(get("/api/cars/price/between/{min}/{max}", 10000, 30000)
                ...
                .andExpect(jsonPath("$.length()").value(3))  // возвращаем 3 элем
    ...
    void testFindCarsPriceBetweenFailure() throws Exception {
        mockMvc.perform(get("/api/cars/price/between/{min}/{max}", 30000, 10000))
                .andExpect(status().isBadRequest())    //1.статус будет BadRequest
                .andExpect(jsonPath("$.length()").value(0));  //2. будет пустое тело
    }
